          ...
          {podeVerArmazem && (
            <>
              ...
              <Link to="/" className={`flex items-center h-11 px-3 rounded-lg transition-all duration-200 ${isActive('/') ? 'bg-blue-600 text-white shadow-md' : 'text-[var(--sidebar-text)] hover:bg-[var(--sidebar-item-hover)] hover:text-[var(--sidebar-text-hover)]'}`}>
                {/* 🚀 ALTERAÇÃO: Ícone corrompido reescrito com rects sólidos (não quebra nunca) */}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 shrink-0 flex items-center justify-between px-6 bg-[var(--header-bg)] border-b border-[var(--header-border)] backdrop-blur-xl z-40">
          <div className="text-[var(--header-text)] text-xs font-semibold tracking-wide uppercase">
            {isActive('/configuracoes') ? 'Configurações do Sistema' : 
             isActive('/mercado-full') ? 'Gestor de Envios Full' : 'WMS Operacional'}
          </div>
        </header>
...
// 🚀 ALTERAÇÃO: RouteGuard agora computa dinamicamente a rota correta de fallback para evitar loop de redirects
function RouteGuard({ permissoesObrigatorias, children }: { permissoesObrigatorias: string[], children: JSX.Element }) {
  ...
  if (!temAcesso) {
    // Redireciona de forma inteligente de volta para a primeira tela que o usuário possui acesso
