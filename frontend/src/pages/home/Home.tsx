...
export default function Home() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const somaTriagem = data.reduce((acc: number, p: any) => {
            ...
            if (tipoPallet === 'PADRAO' || tipoPallet === '') return acc + (p._count?.produtos || 0);
            return acc;
          }, 0);
  ...
  const handleExcluirPalletCard = async (e: MouseEvent, palletId: number, numeroPallet: string) => {
    e.stopPropagation(); 
    const confirmou = await toast.confirm(`Deseja excluir permanentemente a posição "${numeroPallet}" da malha?`);
    if (!confirmou) return;
    try {
      const token = localStorage.getItem('wms_token');
      ...
      if (!response.ok) return toast.error(resultado.error || "Erro ao excluir.");
      toast.success(resultado.mensagem || "Posição removida com sucesso!");
      carregarPallets(); 
    } catch { toast.error("Erro de conexão ao tentar excluir a posição."); }
  };

  const handleExportarExcel = async (e: FormEvent) => {
    e.preventDefault();
    if (!palletSelecionado) return toast.error("Selecione uma opção!");
    ...
    try {
      ...
      let configuracaoFetch: any = {
        ...
        body: JSON.stringify({ palletAlvo: palletSelecionado, nomeArquivo })
      };
      ...
      a.href = url;
      a.download = nomeArquivo ? `${nomeArquivo}.xlsx` : `relatorio-${palletSelecionado}.xlsx`;
      document.body.appendChild(a); a.click(); a.remove(); window.URL.revokeObjectURL(url);
      setIsExcelModalOpen(false); setPalletSelecionado(''); setNomeArquivo('');
    } catch (error: any) { toast.error(error.message); } finally { setCarregandoExcel(false); }
  };

  return (
    <div className="relative min-h-screen bg-transparent text-[var(--text-main)] antialiased overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <MalhaEnderecamento busca={busca} setBusca={setBusca} palletsFiltrados={palletsFiltrados} navigate={navigate} handleExcluirPalletCard={handleExcluirPalletCard} imprimirEtiqueta={imprimirEtiqueta} />
        ...
        <ModalRastreabilidade isOpen={isRastreabilidadeOpen} onClose={() => setIsRastreabilidadeOpen(false)} />
      </div>
    </div>
  );
}