    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-enter" onClick={onClose}>
      <div className="bg-white rounded-xl border border-slate-200 p-6 max-w-lg w-full shadow-2xl flex flex-col max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-start pb-4 border-b border-slate-100 shrink-0">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 transition-colors text-xl leading-none px-2">
            ✕
          </button>
        </div>
        ...
        <div className="overflow-y-auto py-4 pr-2 flex-1 scrollbar-thin">
          {loading ? (
            ...
            <div className="relative pl-6 space-y-6">
              ...
              <div className="absolute top-2 bottom-2 left-[7px] w-[2px] bg-slate-200" />

              {historico.map((mov, index) => (
                <div key={mov.id} className="relative">
                  
                  {/* Bolinha da timeline com anel (ring) para criar o recorte perfeito sobre a linha */}
                  <span className={`absolute -left-[23px] top-1.5 h-3.5 w-3.5 rounded-full ring-4 ring-white shadow-sm z-10 ${
                    index === 0 ? 'bg-blue-600' : 'bg-slate-300'
                  }`} />
                  
                  <div className="bg-slate-50 border border-slate-100 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow relative z-0">
